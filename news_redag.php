<?php
	...
	$news_id = $_GET ['id'];
	$news = mysqli_query($connect, "SELECT * FROM `lastnews` WHERE `id_News` = '$news_id'");
...
<section class="bron_info">
  <div class="employees">
      <div class="container">
        ...
        <center>
    <form class="formm" action="check_redag_news.php" method="post">
		...
		<div class="formm_group">	
			...
			<input type="text" class="inputt" name="title" id="title" placeholder=" " value="<?= $news[title] ?>">
			<br>
		</div>
			
		<div class="formm_group">	
            <label class="labell" > Новина </label>
			<input type="text" class="inputt" name="news" id="news" placeholder=" " value="<?= $news[news] ?>">
			<br>
		</div>	
